...
async function remote(blob, language, fetchImpl, durationMs) {
  const form = new FormData(); form.set('audio', blob, `ptt.${blob.type.includes('ogg') ? 'ogg' : 'webm'}`); form.set('language', language === 'en' ? 'en' : 'pt'); if (durationMs != null) form.set('durationMs', String(durationMs))
...
export async function transcribeAudioFree({ blob, language, durationMs, webSpeechTranscript, fetchImpl = fetch, localTranscriber = transcribeLocalAudio, localEnabled = true, timings = {} }) {
  const failures = []
  try { return { ...(await remote(blob, language, fetchImpl, durationMs)), fallbackDepth: 0, failures } } catch (error) { failures.push(error.code || 'stt_error') }
